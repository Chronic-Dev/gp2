...
void init_usb() {
    ...
    IOUSBDeviceDescriptionSetSerialString(desc, CFSTR("recoveryd"));
    
    CFArrayRef usb_interfaces 
        = (CFArrayRef) IOUSBDeviceDescriptionCopyInterfaces(desc);
    for(i=0; i < CFArrayGetCount(usb_interfaces); i++)
    {
        CFArrayRef arr1 = CFArrayGetValueAtIndex(usb_interfaces, i);
        
        if (CFArrayContainsValue(arr1, 
                CFRangeMake(0,CFArrayGetCount(arr1)), 
                CFSTR("PTP")))
        {
