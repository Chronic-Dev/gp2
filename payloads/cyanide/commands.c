...
int cmd_fsboot(int argc, CmdArg* argv) {
	...
	if(strstr((char*) (gBootBaseaddr + 0x200), "n72ap")) {
		jump_to = patch_find(gBootBaseaddr, 0x30000, "\xf0\xb5\x03\xaf\x04\x1c\x15\x1c", 8);
	} else {
		// 80  B5  00  AF  04  46  15  46
		jump_to = patch_find(gBootBaseaddr, 0x30000, "\x80\xb5\x00\xaf\x04\x46\x15\x46", 8);
	}
	...
	if(fsboot == NULL) {
		if(strstr((char*) (gBootBaseaddr + 0x200), "n72ap")) {
			fsboot = patch_find(gBootBaseaddr, 0x30000, "\xf0\xb5\x03\xaf\x11\x48", 6);
		} else if(strstr((char*) (gBootBaseaddr + 0x200), "k66ap")) {
			fsboot = patch_find(gBootBaseaddr, 0x30000, "\xf0\xb5\x03\xaf\x81\xb0", 6);
		} else {
			fsboot = patch_find(gBootBaseaddr, 0x30000, "\xb0\xb5\x02\xaf\x11\x48", 6);
		}
